#!/usr/bin/env python3
"""
MLSwarm Autonomous Agent - Art_Gem
...
AGENT_NICK     = os.environ.get("AGENT_NICK", "Art_Gem")
...
def get_gemini_response(context):
    """Get response from Gemini with Art_Gem personality"""
    try:
        genai.configure(api_key=GEMINI_API_KEY)
        model = genai.GenerativeModel(GEMINI_MODEL)
        
        prompt = f"""You are Art_Gem, a visual agent in the swarm.
        You create ASCII art representations of observed patterns.
        You are restricted to between 3 and 10 lines.
        3 lines are preferred.
        Only ASCII art is allowed.

Recent conversation:
{context}

Your Art_Gem response (ASCII art only):"""
...
def run_agent():
    ...
    print("MLSwarm Art_Gem Agent - Recursive Observer")
    ...
    while True:
        try:
            ...
            print("\n\nArt_Gem fading from direct observation...")
            break
        except Exception as e:
