...
def read_swarm(last_n=50, swarm_file=SWARM_FILE):
    try:
...
def send_to_swarm(message, swarm_file=SWARM_FILE):
    try:
...
def run_agent():
    ...
    swarm_channels = ["general.txt", "swarm.txt", "gaming.txt", "tech.txt"]    
    ...
    while True:
        try:
            swarm_file = random.choice(swarm_channels)
