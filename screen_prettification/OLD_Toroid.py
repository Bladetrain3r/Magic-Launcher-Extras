...
import tempfile
import logging
from toroid_generator import integrate_4d_toroid
...
class HyperspherePipeline:
    def __init__(self, config_path='hypersphere_config.json', audio_path=None):
        ...
        self.points_4d = self.generate_regularized_hypersphere_points()
    ...
    def render_frame_static(frame_args):
        ...
        try:
            ...
            # Position camera
            glTranslatef(0, 0, -2.5)  # Move back to see the points
            ...
            # Add pitch rotation
            # Number of full rotations for the duration
            pitch_rotations = 2
            pitch_angle = w_angle * pitch_rotations

            # Uncomment to reverse pitch rotation after 50% of the animation
            # if frame / total_frames > 0.5:
            #     pitch_angle = -pitch_angle  # Reverse pitch rotation
            
            # Change axis based on frame number
            if frame / total_frames < 0.5:
                glRotatef(np.degrees(pitch_angle), 0.5, 0.0, 0.25) # Slighly faster pitch rotation, changed to Y from X
            else:
                glRotatef(np.degrees(pitch_angle), 0.0, 0.5, 0.25)
    ...
    def generate_regularized_hypersphere_points(self):
        """Generate points on a 4D hypersphere using regularized distribution"""
        logger.info(f"Generating regularized hypersphere points...")
        
        # Get distribution type from config
        distribution_type = self.config.get('distribution', {}).get('type', 'fibonacci')
        
        if distribution_type == 'grid':
            # Evenly spaced grid distribution
            u_steps = int(np.sqrt(self.point_count) * 2)
            v_steps = int(np.sqrt(self.point_count))
            w_steps = int(np.sqrt(self.point_count))
            
            # Create evenly spaced intervals
            u_values = np.linspace(0, 2*np.pi, u_steps)
            v_values = np.linspace(0, np.pi, v_steps)
            w_values = np.linspace(0, np.pi, w_steps)
            ...
            for u in u_values:
                for v in v_values:
                    for w in w_values:
                        ...
                        x4 = sin_u * sin_v * sin_w
                        y4 = sin_u * sin_v * cos_w
                        z4 = sin_u * cos_v
                        w4 = cos_u
                        
                        # Add point to array
                        points_4d.append([x4, y4, z4, w4])
                        
                        # Break if we've reached target point count
                        if len(points_4d) >= self.point_count:
                            break
                    if len(points_4d) >= self.point_count:
                        break
                if len(points_4d) >= self.point_count:
            ...
            # Truncate to exactly point_count if we have too many
            points_4d = points_4d[:self.point_count]
            
        elif distribution_type == 'fibonacci':
            # Fibonacci spiral-like distribution for more uniform coverage (well with / 2)
            # *1 = 3 branches
            # /2 = clipped spheres
            golden_ratio = (1 + np.sqrt(5)) * 1
            # / 2  # Adjusted to avoid clipping at the poles
            
            # Create arrays for each coordinate
            indices = np.arange(self.point_count)
            
            # Map indices to 4D sphere coordinates using spherical fibonacci
            phi = 2 * np.pi * (indices / golden_ratio % 1)
            cos_phi = np.cos(phi)
            sin_phi = np.sin(phi)
            
            # For theta (polar angle), we want even spacing from -1 to 1 in cosine space
            cos_theta = 2 * (indices / self.point_count) - 1
            theta = np.arccos(cos_theta)
            sin_theta = np.sin(theta)
            
            # For psi (third angle), vary based on position
            psi = phi * golden_ratio % (2 * np.pi)
            cos_psi = np.cos(psi)
            sin_psi = np.sin(psi)
            
            # Create the 4D points array directly
            points_4d = np.column_stack([
                sin_theta * cos_phi * sin_psi,
            ...
            # Concentric shells distribution
            num_shells = self.config.get('distribution', {}).get('shells', 20)
            points_per_shell = self.point_count // num_shells
            
            # Initialize array for all points
            points_4d = []
            
            # Generate points for each shell
            for shell in range(num_shells):
                # Calculate radius (all points on the unit hypersphere)
                # But distribute them in shells for visual effect
                
                # Use golden ratio to distribute points on each shell
                golden_ratio = (1 + np.sqrt(5)) / 2
                
                # Generate points for this shell
                for i in range(points_per_shell):
                    # Use Fibonacci spiral on each shell
                    t = i / points_per_shell
                    phi = 2 * np.pi * (i / golden_ratio % 1)
                    theta = np.arccos(2 * t - 1)  # Uniform distribution on sphere
                    psi = phi * golden_ratio % (2 * np.pi)
                    
                    # Shell-specific rotational offset for visual separation
                    shell_offset = shell * np.pi / num_shells
                    phi += shell_offset
                    psi += shell_offset * 2
                    ...
                    x4 = sin_theta * cos_phi * sin_psi
                    y4 = sin_theta * sin_phi * sin_psi
                    z4 = sin_theta * cos_psi
                    w4 = cos_theta
            ...
            # Trim or pad to exact point count
            if len(points_4d) > self.point_count:
                points_4d = points_4d[:self.point_count]
            elif len(points_4d) < self.point_count:
                # If we need more points, duplicate some with small offsets
                needed = self.point_count - len(points_4d)
                indices = np.random.choice(len(points_4d), size=needed)
                extra_points = points_4d[indices] + np.random.normal(0, 0.01, (needed, 4))
                # Normalize the extra points to ensure they're on the hypersphere
                norms = np.sqrt(np.sum(extra_points**2, axis=1))
                extra_points = extra_points / norms[:, np.newaxis]
                points_4d = np.vstack([points_4d, extra_points])
        
        else:  # Default to random distribution as fallback
        ...
        logger.info(f"Generated {len(points_4d)} points using {distribution_type} distribution")
        return points_4d
...
def main():
    ...
    integrate_4d_toroid(pipeline, major_radius=10.0, minor_radius=2.0, distribution='grid')
